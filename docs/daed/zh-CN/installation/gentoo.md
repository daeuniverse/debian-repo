...
使用稳定关键字的 amd64 系统需要在 `package.accept_keywords` 中添加以下配置。如果 `/etc/portage/package.accept_keywords` 是目录，可写入其中的 `/etc/portage/package.accept_keywords/daed` 文件；如果它是文件，则直接在该文件中添加。
...
检查 Portage 给出的内核配置提示。`webui` USE 标志默认启用；本页介绍的 daed 程序与服务要求保留该标志。禁用后安装的是 dae-wing。配置方法见[上游说明](https://github.com/daeuniverse/daed/blob/main/docs/getting-started.md)。
