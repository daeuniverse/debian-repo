...
If you use `adguardhome`, `mosdns` in `dns` section, refer to [external-dns](/dae/configuration/external-dns).
...
This command requires a firewalld nftables table that permits external changes. With `NftablesTableOwner=yes`, external modification is prohibited; check the [firewalld configuration](https://firewalld.org/documentation/man-pages/firewalld.conf.html) first.
