...
::: warning Unavailable historical dependency
The linked `mount-cgroup2.service` returns HTTP 404, and the current `mailbox.repo` disables its kernel repository by default. These historical commands do not form a verified working upgrade procedure.
:::
> [!WARNING]
